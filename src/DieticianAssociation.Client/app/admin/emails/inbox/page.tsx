...
export default function InboxPage() {
  ...
  }, [fetchInbox, page, pageSize, debouncedSearch, selectedAccount]);
  ...
  const handleEmailPreview = useCallback(
    async (email: SentEmailDto) => {
      ...
      const detailedEmail = await fetchInboxEmailDetail(
        email.id ?? "",
        selectedAccount === "all" ? undefined : selectedAccount,
      );
    ...
    [fetchInboxEmailDetail, selectedAccount],
  );
    ...
    <div className="w-full px-6 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            View all received emails from the system via IMAP.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <Card className="shadow-sm">
        <CardHeader>
          ...
          <CardDescription>
            Filter emails by subject, sender, or account
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card className="shadow-sm">
        <CardHeader>
          ...
          <CardDescription>
            {inboxPagedResult
              ? `${inboxPagedResult.totalItems} total emails`
              : "Loading..."}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {inboxLoading && inboxEmails.length === 0 ? (
            ...
            <>
                      ...
                      <TableRow key={email.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant="outline"
                            className={
                              email.toAddress?.includes("info@")
                                ? "bg-blue-50 text-blue-700 border-blue-200"
                                : email.toAddress?.includes("admin@")
              ...
              {inboxPagedResult && (
                <DataTablePagination
                  currentPage={inboxPagedResult.page ?? page}
                  totalPages={inboxPagedResult.totalPages ?? 0}
                  pageSize={pageSize}
                  totalItems={inboxPagedResult.totalItems ?? 0}
                  hasNextPage={inboxPagedResult.hasNext ?? false}
                  hasPreviousPage={inboxPagedResult.hasPrevious ?? false}
